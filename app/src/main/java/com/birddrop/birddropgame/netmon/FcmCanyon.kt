package com.birddrop.birddropgame.netmon
...
import com.birddrop.birddropgame.notif.GorgeRouter
 ...
 *   * Warm URLs (shell alive) go straight to [PushBusCanyon] and are never saved.
 ...
 */
class FcmCanyon : FirebaseMessagingService() {
    ...
    override fun onNewToken(token: String) {
        ...
        PrefStore(applicationContext).fcmToken = token
    }
    ...
    override fun onMessageReceived(msg: RemoteMessage) {
        ...
        val vault = PrefStore(applicationContext)

        // Warm hand-off works for STREAM users only. NATIVE stays native.
        if (urlOk && vault.runChannel == PrefStore.RunChannel.STREAM &&
            PushBusCanyon.onWarmUrl != null
        ) {
            val delivered = runCatching { PushBusCanyon.handOver(url) }.getOrDefault(false)
            if (delivered) return
        }

        // Cold-start save is also STREAM-only. A NATIVE user gets the text; the
        // launcher never sees the URL and never routes on it.
        val stashUrl = if (urlOk && vault.runChannel != PrefStore.RunChannel.NATIVE) url else ""
    ...
    private suspend fun showNotification(title: String, body: String, url: String, imgUrl: String) {
        ...
        val tap = Intent(ctx, GorgeRouter::class.java).apply {
            flags = Intent.FLAG_ACTIVITY_NEW_TASK or Intent.FLAG_ACTIVITY_SINGLE_TOP
            if (url.isNotBlank()) putExtra(GorgeRouter.EXTRA_PUSH_URL, url)
            putExtra(GorgeRouter.EXTRA_FROM_PUSH, true)
        }
    ...
    companion object {
        private const val TAG = "FcmCanyon"
